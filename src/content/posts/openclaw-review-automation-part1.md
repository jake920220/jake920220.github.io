...
meta_title: "OpenClaw로 서평 자동화 만들기 1부: YES24와 인스타그램 조회 및 신청 설계"
...
## 인스타그램 수집: watchlist 기반의 보수적 접근과 안전 원칙

인스타그램 쪽은 불특정 다수를 무차별 탐색하기보다 접근을 훨씬 보수적으로 잡았다.

알고리즘 피드나 검색 결과를 과도하게 훑는 방식은 불필요한 네트워크 트래픽을 유발하고 플랫폼 이용 정책에도 부합하지 않는다. 그래서 “내가 이미 팔로우하고 있는 관심 출판사 계정만 watchlist에 등록하고, 정해진 주기에 최근 게시물 1~2개만 최소한으로 확인한다”는 구조로 설계했다.

운영 원칙은 아래와 같다.

- 감시 대상은 `memory/instagram-watchlist.json`에 명시된 소수 계정으로 한정
- 실행 주기는 주 2~3회 수준으로 최소화하여 서버 부하 방지
- 계정별로 충분한 대기 간격을 두어 불필요한 연속 요청 방지
- 신규 등록 계정의 경우에도 최근 3개 이내의 최신 게시물만 확인하도록 상한 적용
- 로그인 세션과 쿠키 정보는 외부 유출 없이 로컬 보안 스토리지에 격리 보관

즉, 대량의 데이터를 수집하는 것이 아니라 **내가 챙겨보는 소수 출판사의 모집 소식을 놓치지 않는 최소한의 트리거**가 목표였다.

이 과정에서 가장 중요하게 둔 원칙은 안전과 정책 준수였다.

> 플랫폼의 비정상 신호나 정책 변화가 감지되면 즉시 중단한다.

따라서 로그인 챌린지, 429(Rate Limit), 응답 지연 등의 신호가 감지되면 어떠한 재시도도 하지 않고 워크플로우를 즉각 안전하게 종료하도록 설계했다. 자동화가 지속 가능하려면 시스템의 안전과 플랫폼의 리소스 존중이 최우선이기 때문이다.
...
YES24의 신청은 해당 도서 서평모집 페이지에 신청댓글을 남기는 방식인데, 기존에 작성한 사락 리뷰 링크를 함께 첨부하여 성실한 서평 이력을 전달하는 구조였다.
