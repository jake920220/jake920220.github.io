...
meta_title: "하네스 엔지니어링은 어떻게 개발을 설계 문제로 바꾸는가 | Codex와 GSD 기반 실전 기록"
description: "스핔이 뽀모도로 펫 토이 프로젝트를 진행하며, Codex와 GSD 기반 하네스 엔지니어링이 왜 단순 코드 생성이 아니라 설계, 경계 설정, 검증, ship 단위 운영의 문제인지 정리했다."
...
이번 글은 그 변화를 가볍게 실험해 본 기록이다. 프로젝트는 [**스핔이 뽀모도로 펫**](https://github.com/jake920220/spicky-ppomodoro-pet). 트릭컬의 밈 캐릭터인 스핔이에서 모티브를 가져온 비공식 팬 프로젝트로, macOS와 Windows 위에서 항상 떠 있는 데스크톱 오버레이 펫이자 뽀모도로 타이머를 목표로 단계별 개발을 진행한 토이 프로젝트다.
...
이번 프로젝트는 Vite와 Rust 기반의 **Tauri 2 환경 위에서, 애플리케이션 로직을 Vanilla TypeScript 중심으로 최소화하여 설계**했다. 서버나 거대한 UI 프레임워크를 얹기보다, 내가 집중한 것은 구현보다 **하네스 설계**였다. 다시 말해, Codex가 코드를 안정적으로 작성하도록 만드는 문맥, phase, 검증 기준, ship 단위를 설계하는 일이었다.
...
다섯째, **새로운 기술 스택의 진입장벽은 낮아졌지만 아키텍처 경계 감각의 중요성은 더 커졌다.**
Tauri 환경에서 TypeScript 중심의 단순한 구조만으로도 빠르게 프로젝트를 전개할 수 있었던 이유는, 프레임워크 문법에 매몰되지 않고 문제의 경계를 명확히 나누었기 때문이다.
